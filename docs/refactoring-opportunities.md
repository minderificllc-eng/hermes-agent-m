...
| `agent/conversation_loop.py:523 run_conversation` | 4,939 lines, ~30 scattered `return {...}` exits, `compression_attempts` reset in ≥8 places | **RESOLVED 2026-07-17, all three steps:** (1) ✅ `_finalize_turn()` — all 23 exits, `**extra` preserving each exit's exact key set. (3) ✅ `_try_error_recovery()` — the 12 one-shot recovery arms extracted verbatim (`continue` ↔ `return True`, 1:1 audited); run_conversation is now 4,421 lines. (2) ✅-as-already-done/rejected: the "TurnAttempt object" ALREADY EXISTS as `TurnRetryState` (an upstream refactor collapsed the ~16 one-shot guards); its docstring **deliberately rejects** moving loop-control counters onto it. Critically, `compression_attempts` is TURN-lifetime (init once, reset-on-progress ×8) while `TurnRetryState` is PER-ATTEMPT (fresh each outer iteration) — moving the counter as this entry proposed would reset it every API call, disable the max-3-compressions guard, and produce infinite compression loops on persistent 413s. The ≥8 resets are reset-on-progress semantics, a feature. Do NOT revisit. |
